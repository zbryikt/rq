RQCalendar = function() { return this; };
RQCalendar.prototype = {
  ...
  init: function() {
    ...
    window.addEventListener("click", function(it) { that.popupfunc(); });
  ...
  resize: function() {
    ...
    this.rscale = d3.scale.linear().domain([0,that.config.thresholdIndex,this.maxIndex]).range([5,this.radius/2,this.radius/2]);
  ...
  scroll: function() {
    var that = this;
    var mobile = (this.width < this.rwdbreak);
    var box = this.svg[0][0].getBoundingClientRect();
    if(this.scrollHandler) clearTimeout(this.scrollHandler);
    this.offset = document.body.scrollTop;
    var offset = (-box.top > 0? -box.top : 0);
    if( this.data && offset + window.innerHeight > box.height ) {
      this.handle("lazyloading", this.data[this.data.length - 1].date);
    }
    this.navByDate.attr({
      transform: [
        "translate(",
        (this.width - this.config.margin - this.navDateWidth),
        offset,
      ")"].join(" ")
    });
      ...
      transform: function() {
        return (mobile
          ? [
            ...
            (that.config.margin),
            (parseInt(
              ( offset + window.innerHeight / 4) / (that.weekHeight * 7)
            ) * (that.weekHeight * 7) + window.innerHeight - 70),
            ")"
          ]
          : [
            "translate(",
            (that.width - (that.navDateWidth)),
  ...
  handle: function(name) {
    ...
    for(var i = 0; i < this.handler[name].length; i++) {
      this.handler[name][i]();
    }
  },
  on: function(name, callback) {
    if(!this.handler[name]) this.handler[name] = [];
...
var plotdb = {
  ...
  init: function(chart) {
    chart.root = document.getElementById("container");
    chart.data = [];
    chart.init();
  },
